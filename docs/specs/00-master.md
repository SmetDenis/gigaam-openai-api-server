...
`no_speech_prob=0.0`, а **`compression_ratio` считать честно**: `len(b)/len(zlib.compress(b))`, где `b=text.encode()` (байты с обеих сторон, как в Whisper; для кириллицы `len(text)` занизил бы вдвое)
...
- По мере готовности каждого сегмента: `data: {"type":"transcript.text.delta","delta":"<кусок текста>"}`.
  **Инвариант (как у OpenAI, реализован на этапе 05):** конкатенация всех `delta` точно
  воспроизводит финальный `done.text`. Поэтому разделитель «уезжает» в начало следующего
  delta (первый сегмент — без ведущего пробела, последующие — с пробелом), а НЕ добавляется
  в конец. `"".join(delta) == " ".join(тексты сегментов) == синхронному ответу`.
- В конце: `data: {"type":"transcript.text.done","text":"<полный текст>"}`, затем `data: [DONE]`.
- Heartbeat: на медленном CPU батч считается минутами без событий → периодический
  SSE-комментарий `: keep-alive\n\n` (раз в ~15с) держит соединение против idle-таймаутов прокси.
- Стриминг поддерживается для `response_format` `json`/`text`. Для `verbose_json`/`srt`/`vtt`
  при `stream=true` — **синхронный фоллбэк** (игнорируем `stream`, отдаём полный ответ): эти
  форматы требуют полного результата, а большинство OpenAI-клиентов шлют `stream=true` по
  умолчанию (толерантнее, чем 400; решение этапа 05). Контракт зафиксирован в README.
- Ошибка в середине потока: `data: {"type":"error","error":{...}}` (OpenAI-формат) и закрытие (без `[DONE]`).
...
| `MAX_QUEUE` | int | `8` | Лимит очереди инференса (admitted = в очереди + в работе). Сверх — `503` (этап 04). |
