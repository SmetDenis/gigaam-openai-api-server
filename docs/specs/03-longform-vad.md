...
## Acceptance-критерии
- [x] Аудио >25с распознаётся целиком; результат — несколько сегментов с монотонными границами.
- [x] Таймстемпы слов глобальны (сдвинуты на начало чанка), округлены до 3 знаков.
- [x] `merge_intervals_to_chunks` покрыт юнит-тестами (включая разрезание сверхдлинных и краевые случаи).
- [x] pyannote НЕ импортируется нигде (проверяемо: нет `import pyannote`).
- [~] Пиковая память на длинном файле в рамках 8 ГБ: профиль **обоснован расчётом** (веса ~1 ГБ + int16 ~1.15 ГБ/10ч + float-VAD ~2.3 ГБ/10ч ≈ 4.5 ГБ), float VAD-стадии освобождается до батчей. **Эмпирический замер на реальном ~10ч файле — не делался** (нужен файл + прод-Synology); отложено.
- [x] `make pre-commit` зелёный; интеграционный longform-тест проходит локально.
